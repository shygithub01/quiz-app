  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 38, fontWeight: 700, letterSpacing: 0 }}>
        <span style={{
          background: "linear-gradient(90deg, #668cf8, #00b8d4)",
          WebkitBackgroundClip: "text",
          color: "transparent"
        }}>AI Quiz Generator</span>
      </div>
...
export default function App() {
  ...
  // ... handlers (as before, omitted for brevity) ...
...
const handleSelectOption = (optionIdx: number) => {
  setQuizQuestions(prev => {
    ...
    const isLast = activeQuestion === updated.length - 1;

    if (isLast) {
      const sc = updated.reduce(
        (sum, q) => sum + ((q.userAnswer || "") === q.answer ? 1 : 0),
        0
      );
      setScore(sc);
      setStatus("results");
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f7f8fc" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
        {/* ---- Below is the dynamic content swap ---- */}
        {status === "welcome" && (
          <div style={{
        ...
        {status === "quiz" && quizQuestions.length > 0 && (
          <div style={{
            background: "#fff", borderRadius: 16, boxShadow: "0 2px 22px rgba(0,0,0,0.07)", padding: "38px 30px", minWidth: 360, maxWidth: 500, width: "100%",
            marginTop: 24
          }}>
            <div style={{ color: "#6a7ba2", fontWeight: 500, fontSize: 16, marginBottom: 7 }}>
              Question {activeQuestion + 1} of {quizQuestions.length}
            </div>
            <div style={{ fontSize: 22, fontWeight: 700, margin: "11px 0 19px 0" }}>{quizQuestions[activeQuestion].question}</div>
            <div>
              {quizQuestions[activeQuestion].options.map((op, opIdx) =>
                <button key={opIdx}
        ...
        {status === "results" && (
          <div style={{ background: "#fff", borderRadius: 17, padding: "34px 32px", boxShadow: "0 2px 24px rgba(0,0,0,0.08)", minWidth: 400, maxWidth: 560, marginTop: 24 }}>
