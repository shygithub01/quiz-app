...
import { getFirestore, collection, addDoc, getDocs, doc, updateDoc, deleteDoc, query, orderBy, Timestamp, getDoc } from 'firebase/firestore';
...
// ===== UTILITY FUNCTIONS =====
function generateQuestionHash(questions) {
    return CryptoJS.MD5(JSON.stringify(questions)).toString();
}
const saveQuizTemplate = async (quizTemplate) => {
    try {
...
const getQuizTemplate = async (templateId) => {
    try {
...
const createAttempt = async (userId, quizTemplateId) => {
    try {
...
const updateAttempt = async (userId, attemptId, updates) => {
    try {
...
const saveQuizToFirestore = async (userId, quizData) => {
    try {
...
const updateQuiz = async (userId, quizId, updates) => {
    try {
...
saveQuizTemplate, getQuizTemplate, 
// Quiz Attempt functions
createAttempt, updateAttempt, getUserAttempts, deleteAttempt, getAttemptById, 
// Legacy functions (for backward compatibility)
saveQuizToFirestore, getUserQuizzes, updateQuiz, deleteQuiz };
